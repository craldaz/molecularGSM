...
#if 1
	if (!isMAP_SE) //experimental 12/31
...
//do I need this?
  ...
  for (int n=0;n<nnmax0;n++)
	...
	if (isPRODUCT)
	{
	  ...
	  for (int i=0;i<3*natoms;i++)
	    dgrad[i]= 0.5*(icoords[0].grad1.grada[0][i]- icoords[0].grad1.grada[1][i]);
	  printf(" Calculating sab[%i]\n",1); ///need to initialize
	  for (int i=0;i<3*natoms;i++)
	    sab[i]= 0.5*(icoords[0].grad1.grada[1][i]+icoords[0].grad1.grada[0][i]);
	  //printf(" Calculating dvec[%i]\n",1);
	  //grad1.dvec_calc(coords[0],dvec,runNum,1); //
	  for (int i=0;i<3*natoms;i++)
      dvec[i]=icoords[0].dvec[i];
		
	  for (int i=0;i<nstates-1;i++)
	  {
	    icoords[0].grad1.dE[i] = (icoords[0].grad1.E[i+1] - icoords[0].grad1.E[i])/627.5;
	    printf(" dE[%i]: %5.4f\t Hartree",i,icoords[0].grad1.dE[i]);
	  }
	  printf("\n");
	
	  for (int i=0;i<3*natoms;i++)
	    tmp[i] = dvec[i]*icoords[0].grad1.dE[wstate2-2];
		
	  printf(" Multiplying dvec by delta E to get NACM\n");
	  for (int i=0;i<3*natoms;i++)
	    dvec[i] = dvec[i]*icoords[0].grad1.dE[wstate2-2];
	  ...
	  for (int i=0;i<40;i++)
	  {
	    ...
	    for (int k=1;k<counter+1;k++)
	    {
	      ...
	      for (int i=0;i<natoms;i++)
           ...
           icoords[k].grad1.tc1.readOrb=true;
   	       string cmd = "cp scratch/ORBFILE"+runNameCopy+" scratch/ORBFILE"+runName;
   	       system(cmd.c_str());
        #endif
...
//Set V0
  ...
  nnR = 1;
  nn = 4;
  if (isSSM || isMAP_SE) nn = 2;

  if (isRestart)
		restart_string(strfile0);
  grad1.write_on = 0;
  int nstates = icoords[0].grad1.nstates;
  if ((initialOpt<1 || isRestart) && !isMAP_DE && !isMAP_SE) //&& !isSE_ESSM || !isSSM 
	{
#if !USE_MOLPRO
  ...
  if (!isRestart)
  {
    ...
    if (isSSM || isMAP_SE) active[nnmax-2] = -1;
    if (GROWD==1 && !isSSM && !isMAP_SE)
    {
  ...
  int osteps=STEP_OPT_ITERS;
  if (isFSM)
  {
    osteps = STEP_OPT_ITERS;
    set_fsm_active(1,nnmax-2);
  }
  else if (isSSM || isMAP_SE)
  {
    osteps = STEP_OPT_ITERS;
    if (!isRestart)
      set_fsm_active(1,1);
...
#ifdef _OPENMP
#if QCHEM || QCHEMSF || USE_ORCA || USE_TC
    ...
    if (noptsteps>0 && isSSM)
      V_profile[nnmax-1] = icoords[nnmax-1].opt_b("scratch/lastnode.xyz"+nstr,noptsteps,0,0.0);
		if (noptsteps>0 && isMAP_SE)
  		V_profile[nmax-1] = icoords[nnmax-1].opt_meci(runNum,nnmax,noptsteps)-V0;
		active[nnmax-1]=-2;
...
void GString::get_tangents_1(double** dqa, double* dqmaga, double** ictan)
{
  ...
  for (int n=n0+1;n<nnmax;n++)
  {
    ...
    intic.reset(natoms,anames,anumbers,icoords[n-1].coords);
		newic.copy_CI(icoords[n]);//cra not sure
    newic.update_ic();
    intic.update_ic();
...
//    if (V_profile[n]>V_profile[n-1]+THRESHB) gup++;
	...
	cout << " ispast1 =" << ispast1;
	cout << " ispast2 =" << ispast2;
	cout << " ispast3 =" << ispast3;
#if 0
  ...
  printf("\n");
	...
	double cgrad=0.;
  if (isSSM)
  	cgrad = icoords[nnR-1].gradq[icoords[nnR-1].nicd0-1];
  if (isMAP_SE)
  ...
  for (;oi<max_iter;oi++)
  {
    ...
    for (int i=0;i<nnmax;i++)
      ...
      if (oi>0 && nn < nnmax)
      {
		...
		(isMAP_DE && icoords[nnmax-nnP].grad1.dE[wstate2-2]<2.5 && icoords[nnmax-nnP].gradrms<gaddmax/2.) || \
		(isMAP_DE && icoords[nnR-1].grad1.dE[wstate2-2]<5. && icoords[nnR-1].gradrms<gaddmax/3. &&GROWD!=1) )
		{
      if (oi>0 && nn < nnmax)
      {
        addednode = add_seam_node(nnmax-nnP,nnmax-nnP-1,nnR-1);
        nnP++;
				get_tangents_1g(dqa,dqmaga,ictan);//experimental 7/18
			}
		}
	 ...
	 //MAP_SE add node
      ...
      if (oi>0 && nn < nnmax)
      {
		...
		//add Nodes
    if((icoords[nnR-1].gradrms<gaddmax && GROWD!=2 && !isMAP_DE && !isMAP_SE) || isFSM || isSSM )
    {
      if (oi>0 && nn < nnmax)
      {
        ...
        else //continue opting because single ended
        {
          set_fsm_active(nnR-1,nnR-1); 
      ...
      if (isFSM)
      {
        ...
        opt_steps(dqa,ictan,osteps,oesteps,0,K);
			...
			else if (isMAP_DE)
			{
				...
				{
					active[n]=-2;
          //icoords[n].gradrms> gaddmax)||(icoords[n].grad1.dE[wstate2-2]>2.5 && icoords[n].gradrms>gaddmax/2.) || ( icoords[n].grad1.dE[wstate2-2]>5. && icoords[nnR-1].gradrms>gaddmax/3.))
    ...
    gradrms = 0.;
    for (int i=1;i<nnmax-1;i++)
    if (active[i]>-1 || isSSM || isFSM || isMAP_SE || isMAP_DE)
    { 
    ...
    nmax = 1;
    for (int i=0;i<nnmax;i++)
    if (active[i]>-1 || isFSM || isSSM || isMAP_DE || isMAP_SE)
    if (V_profile[i]>emax)
    {
      emax = V_profile[i];
      nmax = i;
    ...
    else if (!isMAP_DE && !isMAP_SE)
    {
      ...
      cout <<endl;
   	...
   	if (isSE_ESSM && (icoords[nnR-1].grad1.dE[wstate2-2]-icoords[nnR-2].grad1.dE[wstate2-2])>0.0 && (1.0 - icoords[nnR-1].bdist/bdist0)>0.5)
   {
    ...
    if (isMAP_SE)
    {
      double cgrad = icoords[nnR-1].gradq[icoords[nnR-1].nicd0-3];
      printf(" cgr: %6.3f",cgrad);
    }
    printf(" \n");
...
//    print_string_clump(nnmax,totalgrad,allcoords,strfile);
     ...
     int ntotal=0;
     for (int n=0;n<nnmax;n++)
        ntotal+=icoords[n].grad1.nscffail;
     if (ntotal>25)
      {
        printf(" too many failed\n");
				exit(-1);
			}
	addednode=0;//cra
  } // growth iters

  if (tscontinue==1 && (isSSM || isMAP_SE))
  {
    if (pastts==1)
    {
			int addedn=0;
...
void GString::add_last_seam_node(int type)
{
  int nstates = icoords[0].grad1.nstates;
  int wstate = icoords[0].grad1.wstate;
	int wstate2 = icoords[0].grad1.wstate2;
  ...
  string nstr = StringTools::int2str(runNum,4,"0");
  icoords[nnR].grad1.update_knnr();
  V_profile[nnR] = icoords[nnR].opt_meci(runNum,nnR,noptsteps)-V0;
...
// Form the initial string
  ...
  for (int n=2;n<nnodes;n++)
  {
    ...
    icoords[iN].bmatp_to_U();
    icoords[iN].bmat_create();
		V_profile[iN]=icoords[iN].calc_BP(runNum,iN)-V0;
...
void GString::opt_iters_seam(int max_iter, double& totalgrad, double& gradrms, string strfileg,int osteps, int oesteps,double** dqa, double* dqmaga, double** ictan)
{
  ...
  for (;oi<max_iter;oi++)
    ...
    printf(" opt_iter: %2i totalgrad: %1.3f gradrms: %1.4f tgrads: %i",oi+1,totalgrad,gradrms,gradJobCount);
		...
		if (totalgrad<(CONV_TOL*(nnmax-2)*rn3m6*5))
		{
				printf(" Seam is converged.\n");
  			string strfile = "stringfile.xyz"+nstr;
				printf(" printing final string to %s \n",strfile.c_str());
		...
		{		
			printf(" seam convergence criteria is %1.5f\n",CONV_TOL*(nnmax-2)*rn3m6*5);
		}
  ...
  if (icoords[0].grad1.nstates>1)
	{
  	...
  	for (int n=0;n<nodes;n++)
		...
		{	
  	  	...
  	  	xyzfilec<< icoords[n].grad1.dE[icoords[0].grad1.wstate-2] << endl;      
		}
  	...
  	for (int n=0;n<nodes;n++)
		if (active[n]>-1 || active[n]==-2 || isSSM || isMAP_SE)
		{
			if (icoords[0].grad1.wstate2>0)
