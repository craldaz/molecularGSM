  ...
  printf("\n");
	...
	for (int i=0;i<len;i++)
		for (int j=0;j<nicd0;j++)
			dgrad_U[i]+=dgradq[j]*Ut[j*len+i]/norm_dg;
...
void ICoord::bp_rot(double* C)
{
	...
	double norm_dg1=0.;
	for (int i=0;i<len;i++)
		norm_dg1+=dgrad_U[i]*dgrad_U[i];
	norm_dg1=sqrt(norm_dg1);
	double norm_dv=0.;
	for (int i=0;i<len;i++)
...
void ICoord::model_CI(int run, int node)
{
  ...
  V0=grad1.grads(coords, grad, Ut, 3);
  ...
  for (int i=0;i<40;i++)
  {
    ...
    {
      for (int i=0;i<3*natoms;i++)
        r[i]=coords[i]+rmag*cos(theta[min[k]])*dgrad[i]/norm_rdgrad + rmag*sin(theta[min[k]])*dvec[i]/norm_rnacm;
      for (int i=0;i<3*natoms;i++)
        tmp[i]=coords[i];
      for (int i=0;i<3*natoms;i++)
        coords[i]=r[i];
