...
// ShaoYu modified for frequency caculation //
  inpfile << "PM7 GRAPHF GNORM=" << gnorm << " UHF CHARGE=" << charge << " AUX BONDS DENSITY PI ENPART MMOK " << endl;
...
double Mopac::opt(string filename, ICoord icoords) {
  ...
  inpfile << "    "  << endl;
  inpfile << "OLDGEO FORCE PRECISE THERMO ROT=1 PM7 GRAPHF AUX BONDS DENSITY PI ENPART MMOK" << endl;
 ...
 inpfile << "    "  << endl;
  inpfile << "OLDGEO FORCE PRECISE THERMO ROT=1 PM7 GRAPHF AUX BONDS DENSITY PI ENPART MMOK" << endl; 
  //string cmd = "~/MOPAC2016.exe "+filename;
